...
export default function AvoirsPageWrapper() {
  return (
...
function AvoirsPage() {
  ...
  const { data, isLoading, refetch } = useQuery({
    queryKey: ["factures-avoirs", page, perPage, search, statutFilter],
    queryFn: async () => {
      const response = await facturesService.getAll({
        page,
        per_page: perPage,
        search: search || undefined,
        type: "avoir_client,avoir_fournisseur",
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ["factures-avoirs"] });
      refetch();
  ...
  const handleStatutChange = (statut: string) => { setStatutFilter(statut); setPage(1); };
  const handleDeleteClick = (item: EcritureComptable) => { setItemToDelete(item); setDeleteDialogOpen(true); };
  ...
  const actions = [
    ...
    { label: "Supprimer", icon: <Trash2 className="h-4 w-4" />, onClick: (item: EcritureComptable) => handleDeleteClick(item), variant: "ghost" as const, className: "text-red-600 hover:text-red-700 hover:bg-red-50 dark:text-red-400 dark:hover:bg-red-950/30" },
  ];
    ...
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Gestion des avoirs clients et fournisseurs</p>
        </div>
        <Button onClick={() => router.push("/dashboard/factures/nouveau")} className="bg-blue-600 hover:bg-blue-700">
          <Plus className="h-4 w-4 mr-2" />Nouvel avoir
        </Button>
      </div>
      ...
      <Card>
        <CardHeader className="pb-2">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
            <CardTitle className="text-sm font-medium text-gray-500 dark:text-gray-400">{(data as any)?.total ?? 0} avoir(s)</CardTitle>
      ...
      <ConfirmDialog
        ...
        description={`Êtes-vous sûr de vouloir supprimer l'avoir "${itemToDelete?.reference || ''}" ? Cette action est irréversible.`}
