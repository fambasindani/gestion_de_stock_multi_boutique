...
export default function DashboardPage() {
  const { currentUser, isLoadingUser: isUserLoading } = useAuth();
        ...
        <div className="relative z-10 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-xl md:text-2xl font-bold text-white">
                Bonjour, {currentUser?.nom?.split(" ")[0] || "Utilisateur"}
              </h1>
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 text-sm text-slate-300 bg-white/5 hover:bg-white/10 rounded-xl transition-all border border-white/10"
            >
              {showStats ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}
              {showStats ? "Masquer" : "Afficher"}
            </button>
            <div className="flex items-center gap-2.5 px-4 py-2 bg-emerald-500/10 rounded-xl border border-emerald-500/20">
